...
Schema :: Schema (Schema * a, Schema * b)
{
    numAtts = a->GetNumAtts() + b->GetNumAtts();
    myAtts = new Attribute[numAtts];
    Attribute * p_a = a->GetAtts();
    for(int i = 0; i< a->GetNumAtts(); i++){
	if (p_a[i].myType == Int) {
	    myAtts[i].myType = Int;
	}
	else if (p_a[i].myType == Double) {
	    myAtts[i].myType = Double;
	}
	else if (p_a[i].myType == String) {
	    myAtts[i].myType = String;
	} 
	else {
	    cout << "Bad attribute type for " << p_a[i].myType << "\n";
	    delete [] myAtts;
	    exit (1);
	}
	myAtts[i].name = strdup (p_a[i].name);
    }
    Attribute * p_b = b->GetAtts();
    int numA =  a->GetNumAtts();
    for(int i = numA; i< numAtts; i++){
	if (p_b[i-numA].myType == Int) {
	    myAtts[i].myType = Int;
	}
	else if (p_b[i-numA].myType == Double) {
	    myAtts[i].myType = Double;
	}
	else if (p_b[i-numA].myType == String) {
	    myAtts[i].myType = String;
	} 
	else {
	    cout << "Bad attribute type for " << p_b[i-numA].myType << "\n";
	    delete [] myAtts;
	    exit (1);
	}
	myAtts[i].name = strdup (p_b[i-numA].name);
    }
}
