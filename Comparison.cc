...
void OrderMaker :: serializer(ofstream &out){
  out << numAtts <<endl; 
...
void OrderMaker :: deserializer(ifstream &in){
  in >> numAtts;
  for(int i=0;i<MAX_ANDS;i++)
    in >> whichAtts[i];
  int types;
...
OrderMaker :: OrderMaker(OrderMaker * o) {
	numAtts = o->numAtts;
	for(int i=0;i<MAX_ANDS;i++){
	  whichAtts[i] = o->whichAtts[i];
	  whichTypes[i] = o->whichTypes[i];
	}
}
	...
	for (int whichAnd = 0; 1; whichAnd++, parseTree = parseTree->rightAnd) {
		...
		for (int whichOr = 0; 1; whichOr++, myOr = myOr->rightOr) {
			...
			if (myOr->left->left->code == NAME) {
				...
				// it is not there!  this disjunction should be ignored
                                } else {
